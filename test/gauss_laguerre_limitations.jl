...
@testset "Gauss-Laguerre limitations for Exponential" begin
    ...
    E_laguerre = expectation(dist, Gaussian, n=32)
    @test E_laguerre(x -> x) ≈ mean(dist) rtol=1e-10  # Mean works well
    @test E_laguerre(x -> x^2) ≈ 2*θ^2 rtol=1e-10     # Second moment works well
    ...
    result_laguerre = E_laguerre(step_func)
    # Gauss-Laguerre has significant error (>10% relative error)
    @test abs(result_laguerre - analytical) > 0.05  # Error is substantial
    ...
    @test abs(result_quant - analytical) < abs(result_laguerre - analytical)  # Better than Gauss-Laguerre
    
    # Test 4: Show node distribution issue
    # Gauss-Laguerre places most nodes in the tail
    nodes_laguerre = nodes(E_laguerre)
    @test sum(nodes_laguerre .< θ) < 5  # Less than ~15% of nodes below mean
    @test sum(nodes_laguerre .> θ) > 27  # More than ~85% of nodes above mean
